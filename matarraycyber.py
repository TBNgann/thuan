...
class Zefoy:
	...
	def __init__(self):
		...
		self.url = 'None'
		self.text = 'Array Cyber'
		url1=input("[ArrayCyber]</> Nhập link video: ")
		self.url=url1
	...
	def get_captcha(self):
		...
		try:
			...
			print('[ArrayCyber]</> Đang giải capcha..')
			return False
		except Exception as e:
			print(f"[ArrayCyber]</> Không thể giải captcha: {e}")
			time.sleep(2)
			self.get_captcha()

	def send_captcha(self, new_session = False):
		if new_session: self.session = requests.Session(); os.remove('session'); time.sleep(2)
		if self.get_captcha(): print('[ArrayCyber]</> Đang kêt nối đến session');return (True, 'The session already exists')
		...
		if 'Enter Video URL' in request.text: 
			print('[ArrayCyber]</> Session đã được tạo')
			open('session','w',encoding='utf-8').write(self.session.cookies.get('PHPSESSID'))
			print(f"[ArrayCyber]</> Giải capcha thành công: {captcha_solve}")
	...
	def find_video(self):
		...
		while True:
			...
			if 'Session expired. Please re-login' in self.video_info: print('[ArrayCyber]</> Phiên hết hạn. Đang đăng nhập lại...');self.send_captcha(); return
			elif 'service is currently not working' in self.video_info: return (True,'[ArrayCyber]</> Dịch vụ hiện không hoạt động, hãy thử lại sau. | Bạn có thể thay đổi nó trong cấu hình.')
				...
				elif lamtilo >= 1000: print('[ArrayCyber]</> Your IP was banned')
				# print(f'[ArrayCyber]</> Thời gian chờ để tiếp tục tăng: {t} giây')
				# for lam in range(t,0,-1):
				# 	lam-=1
				# 	print(f"[ArrayCyber]</> Vui lòng chờ: {lam}",end="\r")
				# 	time.sleep(1)
				_=time.time()
				while time.time()-2<_+lamtilo:
					t-=1
					print("[ArrayCyber]</> Vui lòng chờ: {0} ".format(t)+"giây .", end="\r")
					time.sleep(1)
	...
	def use_service(self):
		...
		if 'Session expired. Please re-login' in res: print('[ArrayCyber]</> Phiên hết hạn. Đang đăng nhập lại...');self.send_captcha(); return ""
		elif 'Too many requests. Please slow' in res: time.sleep(3)
		elif 'service is currently not working' in res: return ('[ArrayCyber]</> Dịch vụ hiện không hoạt động, hãy thử lại sau. | Bạn có thể thay đổi nó trong cấu hình.')
	...
	def get_video_id(self, url_ = None, set_url=True):
		...
		if request.text == '': print('[ArrayCyber]</> Link video không hợ lệ'); return False
	...
	def update_name(self):
		while True:
			try:
				...
				self.text = f"[ArrayCyber]</> | Views: {video_info['viewCount']} "
			except: pass
			time.sleep(5)
trang = "\033[1;37m"
xanh_la = "\033[1;32m"
...
def logo():
    ...
    kt_code = "</>"
    logo=f"""
{xanh_duong} _ _______ _______ _ ____ ____ ______ ____ ____ ______ ________ _______ / \ |_ __ \ |_ __ \ / \ |_ _||_ _| .' ___ ||_ _||_ _||_ _ \ |_ __ ||_ __ \ / _ \ | |__) | | |__) | / _ \ \ \ / / / .' {trang}\_| \ \ / / | |_) | | |_ \_| | |__) | / ___ \ | __ / | __ / / {vang}___ \ \ \/ / | | \ \/ / | __'. | _| _ | __ / _/ / \ \_ _| | \ \_ {xanh_la}_| | \ \_ _/ / \ \_ _| |_ \ `.___.'\ _| |_ _| |__) |_| |__/ | _|  {tim}| \ \_ |____| |____||____| |___||____| |___||____| |____|| {dac_biet}______| `.____ .' |______| |_______/|________||____| |___| 
 {xanh_la} </> Đăng kí kênh Telegram : https://t.me/ArrayCyber để có nhiều tool vip nhé! </>
    \033[1;37m────────────────────────────────────────────────────────────
    {do}[{trang}{kt_code}{do}] {vang}TOOL BUFF Views TIKTOK Array Cyber Dev
    {do}[{trang}{kt_code}{do}] \033[1;35mADMIN:\033[1;36m https://t.me/Vanduy0982
    {do}[{trang}{kt_code}{do}] \033[1;31mZALO: \033[1;33m Đang Cập Nhập
    {do}[{trang}{kt_code}{do}] {dac_biet}Telegram :\033[1;37m https://t.me/ArrayCyber
    {do}[{trang}{kt_code}{do}] {xanh_la}FACEBOOK:\033[1;37m Array Cyber 
    \033[1;37m────────────────────────────────────────────────────────────\n"""
...
#     print(f"{trang}[ArrayCyber]</> Link key: {link1}")
     
# else:
#     link1=""
# nhap = input(f"{trang}[ArrayCyber]</> Nhập key vừa lấy được: ")
# if nhap == key:
if 1:
    print(f"{xanh_la}[ArrayCyber]</> Key đúng mời bạn vô tool")
    logo()
    Z = Zefoy()
    ...
    print(f"{do}[ArrayCyber]</> Key sai vui lòng vượt lại")
    exit()
